...
namespace CML {
  ...
  void EDFSave::StartFile_Handler(RC::RStr& filename) {
    ...
    for (size_t c=0; c<chan_cnt; c++) {
      channels[c] = hndl->elec_config->data[c][1] - 1;
    }
    ...
    if (edf_set_equipment(edf_hdl, 0, "Elemem using Blackrock NeuroPort")) {
      Throw_RC_Type(File, "Could not set edf equipment");
    }
    
    std::string sub_name;
    hndl->eeg_acq.exp_config.Get(sub_name, "subject");
  ...
  void EDFSave::SaveData_Handler(RC::APtr<const EEGData> data) {
    ...
    for (size_t c=0; c<data.size(); c++) {
      buffer[c] += (*data)[c];
      max_written = std::max(max_written, (*data)[c].size());
    }
    ...
    while (amount_buffered > sampling_rate) {
      ...
      for (size_t c=0; c<channels.size(); c++) {
        if (c >= buffer.size()) {
          StopSaving_Handler();
          Throw_RC_Type(File, ("EDF save, configured channel " + RStr(c+1) + 
                " out of bounds").c_str());
        }
        ...
        if (buffer[channels[c]].size() < sampling_rate) {
          StopSaving_Handler();
          Throw_RC_Type(File,
              ("Data missing on edf save, channel " + RStr(c+1)).c_str());
        }
