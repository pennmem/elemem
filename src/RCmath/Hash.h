#ifndef RCMATH_HASH_H
#define RCMATH_HASH_H
...
class Hash {
  ...
  #define RC_WP_DIGESTBYTES 64
  #define RC_WP_DIGESTBITS  (8*RC_WP_DIGESTBYTES) /* 512 */

  #define RC_WP_BLOCKBYTES 64
  #define RC_WP_BLOCKBITS  (8*RC_WP_BLOCKBYTES) /* 512 */

  #define RC_WP_LENGTHBYTES 32
  #define RC_WP_LENGTHBITS  (8*RC_WP_LENGTHBYTES) /* 256 */

  typedef struct NESSIEstruct {
    u8  bitLength[RC_WP_LENGTHBYTES]; /* global number of hashed bits (256-bit counter) */
    u8  buffer[RC_WP_BLOCKBYTES];  /* buffer of data to hash */
    int bufferBits;           /* current number of bits on the buffer */
    int bufferPos;            /* current (possibly incomplete) byte slot on the buffer */
    u64 hash[RC_WP_DIGESTBYTES/8];    /* the hashing state */
  } NESSIEstruct, WPstate;
   ...
   *
   * ***********************************************
   * ***  Code modified by RC, still compliant.  ***
   * ***********************************************
   *
   */
   ...
   */
  static const int whirlpool_rounds = 10;
  ...
  inline static const u64* C0() { static u64 x[256] = {
  ...
  inline static const u64* C1() { static u64 x[256] = {
  ...
  inline static const u64* C2() { static u64 x[256] = {
  ...
  inline static const u64* C3() { static u64 x[256] = {
  ...
  inline static const u64* C4() { static u64 x[256] = {
  ...
  inline static const u64* C5() { static u64 x[256] = {
  ...
  inline static const u64* C6() { static u64 x[256] = {
  ...
  inline static const u64* C7() { static u64 x[256] = {
  ...
  inline static const u64* rc() { static u64 x[whirlpool_rounds+1] = {
   ...
   */
  static void NESSIEinit(NESSIEstruct * const structpointer) {
      int i;
   ...
   *
   * This method maintains the invariant: bufferBits < RC_WP_DIGESTBITS
   */
  static void NESSIEadd(const unsigned char * const source,
                 size_t sourceBits,
      ...
      */
      ...
      for (i = 31, carry = 0; i >= 0 && (carry != 0 || value != UINT64_C(0)); i--) {
      ...
      while (sourceBits > 8) {
          ...
          if (bufferBits == RC_WP_DIGESTBITS) {
              /*
               * process data block:
               */
   ...
   * 
   * This method uses the invariant: bufferBits < RC_WP_DIGESTBITS
   */
  static void NESSIEfinalize(NESSIEstruct * const structpointer,
      ...
      /*
       * pad with zero bits to complete (N*RC_WP_BLOCKBITS - RC_WP_LENGTHBITS) bits:
       */
      if (bufferPos > RC_WP_BLOCKBYTES - RC_WP_LENGTHBYTES) {
          if (bufferPos < RC_WP_BLOCKBYTES) {
              memset(&buffer[bufferPos], 0, RC_WP_BLOCKBYTES - bufferPos);
          }
          /*
           * process data block:
           */
      ...
      if (bufferPos < RC_WP_BLOCKBYTES - RC_WP_LENGTHBYTES) {
          memset(&buffer[bufferPos], 0, (RC_WP_BLOCKBYTES - RC_WP_LENGTHBYTES) - bufferPos);
      }
      bufferPos = RC_WP_BLOCKBYTES - RC_WP_LENGTHBYTES;
      /*
       * append bit length of hashed data:
       */
      memcpy(&buffer[RC_WP_BLOCKBYTES - RC_WP_LENGTHBYTES], bitLength, RC_WP_LENGTHBYTES);
      /*
       * process data block:
       */
      processBuffer(structpointer);
       ...
       */
      for (i = 0; i < RC_WP_DIGESTBYTES/8; i++) {
  ...
  inline static u8* fillbuf() { static u8 x[128] =
    { 0x80, 0 /* , 0, 0, ...  */ };
    return x;
  }
  ...
  inline static const u64* K() { static u64 x[80] = {
  ...
  static void sha512_process_block (const void *buffer, size_t len,
  ...
  #define RC_SHA_CH(x, y, z) ((x & y) ^ (~x & z))
  #define RC_SHA_MAJ(x, y, z) ((x & y) ^ (x & z) ^ (y & z))
  #define RC_SHA_S0(x) (RC_SHA_CYCLIC (x, 28) ^ RC_SHA_CYCLIC (x, 34) ^ RC_SHA_CYCLIC (x, 39))
  #define RC_SHA_S1(x) (RC_SHA_CYCLIC (x, 14) ^ RC_SHA_CYCLIC (x, 18) ^ RC_SHA_CYCLIC (x, 41))
  #define RC_SHA_R0(x) (RC_SHA_CYCLIC (x, 1) ^ RC_SHA_CYCLIC (x, 8) ^ (x >> 7))
  #define RC_SHA_R1(x) (RC_SHA_CYCLIC (x, 19) ^ RC_SHA_CYCLIC (x, 61) ^ (x >> 6))

      /* It is unfortunate that C does not provide an operator for
         cyclic rotation.  Hope the C compiler is smart enough.  */
  #define RC_SHA_CYCLIC(w, s) ((w >> s) | (w << (64 - s)))

      /* Compute the message schedule according to FIPS 180-2:6.3.2 step 2.  */
      for (t = 0; t < 16; ++t) {
        W[t] = RC::Endian::ToBig (*words);
        ++words;
      }
      for (t = 16; t < 80; ++t) {
        W[t] = RC_SHA_R1 (W[t - 2]) + W[t - 7] + RC_SHA_R0 (W[t - 15]) + W[t - 16];
      }

      /* The actual computation according to FIPS 180-2:6.3.2 step 3.  */
      for (t = 0; t < 80; ++t) {
        uint64_t T1 = h + RC_SHA_S1 (e) + RC_SHA_CH (e, f, g) + K()[t] + W[t];
        uint64_t T2 = RC_SHA_S0 (a) + RC_SHA_MAJ (a, b, c);
        h = g;
        g = f;
        f = e;
        e = d + T1;
  ...
  static void * sha512_finish_ctx (struct sha512_ctx *ctx, void *resbuf) {
    ...
    u64 total0 = RC::Endian::ToBig (ctx->total[0] << 3);
    u64 total1 = RC::Endian::ToBig ((ctx->total[1] << 3) |
                                    (ctx->total[0] >> 61));
    memcpy(ctx->buffer + bytes + pad + 8, &total0, sizeof(total0));
    memcpy(ctx->buffer + bytes + pad, &total1, sizeof(total0));
    ...
    for (i = 0; i < 8; ++i) {
      ((uint64_t *) resbuf)[i] = RC::Endian::ToBig (ctx->H[i]);
    }
  ...
  /* To check alignment gcc has an appropriate operator.  Other
     compilers don't.  */
  # if __GNUC__ >= 2
  #  define RC_SHA_UNALIGNED_P(p) (((uintptr_t) p) % __alignof__ (uint64_t) != 0)
  # else
  #  define RC_SHA_UNALIGNED_P(p) (((uintptr_t) p) % sizeof (uint64_t) != 0)
  # endif
      if (RC_SHA_UNALIGNED_P (buffer)) {
        while (len > 128) {
  ...
  //  C++ wrapper:
  inline static void WPinit(WPstate &s) {
    NESSIEinit(&s);
  }

  inline static void WPadd(WPstate &s, const RC::Data1D<u8>& data) {
    NESSIEadd(data.Raw(), data.size() << 3, &s);
  }

  inline static void WPdone(WPstate &s, RC::Data1D<u8>& hash) {
    hash.Resize(64);
    NESSIEfinalize(&s, hash.Raw());
  }
  ...
  static void Whirlpool(const RC::Data1D<u8>& data, RC::Data1D<u8>& hash) {
    WPstate s;
    WPinit(s);
    WPadd(s, data);
    WPdone(s, hash);
  }
  static RC::Data1D<u8> Whirlpool(const RC::Data1D<u8>& data) {
    RC::Data1D<u8> hash;
    Whirlpool(data, hash);
    return hash;
  }
  ...
  static void SHA512(const RC::Data1D<u8>& data, RC::Data1D<u8>& hash) {
    struct sha512_ctx ctx;
    hash.Resize(64);

    sha512_init_ctx(&ctx);
    sha512_process_bytes(data.Raw(), data.size(), &ctx);
    sha512_finish_ctx(&ctx, hash.Raw());
  }
  static RC::Data1D<u8> SHA512(const RC::Data1D<u8>& data) {
    RC::Data1D<u8> hash;
    SHA512(data, hash);
    return hash;
  }
  ...
  static void HashX(const RC::Data1D<u8>& data, RC::Data1D<u8>& hash) {
    RC::Data1D<u8> temphash;

    SHA512(data, temphash);
    Whirlpool(data, hash);

    for (size_t i=0; i<hash.size(); i++) {
      hash[i] = hash[i] ^ temphash[i];
    }
  }
  static RC::Data1D<u8> HashX(const RC::Data1D<u8>& data) {
    RC::Data1D<u8> hash;
    HashX(data, hash);
    return hash;
  }
  ...
  // Adds log2(strength) bits of hardening, plus requires strength*64 bytes
  // of memory to protect against specialized hardware attacks.
  static RC::Data1D<u8> HardenX(u64 strength, const RC::Data1D<u8>& data,
                         const RC::Data1D<u8>& salt=RC::Data1D<u8>()) {
    RC::Data1D<u8> hash(64);
    RC::Data1D<u8> hstep(64+sizeof(u64));
    RC::Data1D<u8> tempstore(128);

    strength = Max(strength, (u64)1);
    RC::Data1D<u8> bigarray(64*strength);

    RC::Data1D<u8> passsalt = salt;
    passsalt += data;

    HashX(passsalt, bigarray);
    hash = bigarray;
    for (u64 s=1; s<strength; s++) {
      hstep.CopyAt(0, bigarray);
      hstep.Put(s-1, 64);
      bigarray.SetOffset(s * 64);
      HashX(hstep, bigarray);
    }

    for (u64 s=strength; s>0; s--) {
      bigarray.SetOffset((s-1)*64);
      bigarray.Resize(64);
      tempstore = hash;
      tempstore += bigarray;
      HashX(tempstore, hash);
    }
